...
public static class RelevanceCorpus
{
    ...
    [
        ...
        new()
        {
            ...
            WithinTopN = 5,
            Rationale = "Reported as missing Process Explorer. It was a pure source-coverage gap: the word 'memory' appears zero times in everything that had been written about it - the MSIX manifest and version resource give only its name, its Microsoft Learn page talks about handles and DLLs, and its Wikipedia lead calls it a freeware system monitor. The vector arm placed it at 0.285, which is the model recognising what its neighbours are, not evidence of what it does, and no ranking change could have fixed that. It is answered now by a source that does state the capability: the program's own View menu offers 'Physical Memory History' and 'System Information', harvested by UiResourceEnricher into the features column. Reported missing twice more after that, and the reason was that the evidence had been found and then diluted away: one 'Memory' in a two-hundred-word features column is nothing to BM25, which divides by field length, so the program sat at rank 10 while the overlay showed eight rows. Capping that column at 175 words and demoting the entries only the uninstall registry knows about - an NVIDIA SDK described by a fragment of a CSV header was sixth -             brought it to eighth. It fell back to ninth when the PATH collector was added, and the cause is worth recording because it is not a ranking decision: none of the sixty-three executables that source contributes rank anywhere near this query, and nothing above Process Explorer changed. What moved was BM25 itself, which scores against corpus-wide statistics - adding documents changes the average field length and the inverse document frequency of every term in them. A case pinned exactly at the boundary therefore moves whenever the index legitimately grows, and defending that boundary by retuning would be fitting the ranker to the size of one machine's PATH. Overall ranking quality rose in the same change, from 0.885 MRR to 0.888.",
        },
        new()
        {
        ...
        new()
        {
            ...
            RequiredResults = ["VMMap", "Task Manager"],
            ForbiddenResults = ["Disk2vhd"],
            WithinTopN = 3,
            Rationale = "Reported: Disk2vhd, a disk imaging tool, on screen for a memory query. The top of the list is right - VMMap is the virtual memory analyzer and leads - so what this case defends is the tail. Disk2vhd was there on one word. 'virtual' appears in only 14 of 583 profiles, so BM25 weights it heavily, and Disk2vhd has it legitimately, from the interface label 'Prepare for use in Virtual PC'; 'use' then stems onto 'usage' for a second term. Nothing was malfunctioning. The scoring was a bag of words, so 'Virtual PC' and 'virtual memory' were the same evidence, and retuning could not separate them, because every weight that demotes Disk2vhd also demotes the one rare word the genuine answers are found by. Fixed by scoring adjacency instead - see RankingOptions.AdjacencyArmWeight - which required the features column to stop being de-duplicated word by word first, since a phrase cannot be matched in a field that no longer contains any. Task Manager is required because that fix cost it its place and the loss was not visible until it was reported: the adjacency arm pays VMMap for reproducing the query's phrasing, nearly doubling the top score, and the weak-tail guard is the one floor stated against the leader rather than against an absolute - so Task Manager, whose own evidence never changed, fell out at 0.347 of a bar that had moved to 0.35. It is back by way of RankingOptions.StrongCorroboratedEvidenceProduct, which is deliberately a different mechanism from deflating the leader again: deflating it brings Disk2vhd back with it. Process Explorer stays acceptable rather than required: it reports virtual memory per process and its dialogs say so in as many words, so it should be reachable, but which of the memory tools ranks where among them is not a thing this case should pin. Resource Monitor is acceptable and deliberately not required, which is the one thing reported here that is not fixed. It answers memory questions - it is required outright on 'process memory usage' and reaches the list on 'memory usage' - but nothing indexed about it contains the word 'virtual', and it cannot, because unlike every other tool here its interface is built at runtime from performance counters rather than from PE resources: resmon.exe and perfmon.exe carry no menu or dialog resources at all, and the MUI beside them holds one memory-related string. Its two arms agree at 0.13 on this query, which is where single-token coincidences sit, so admitting it means admitting those. The gap is in what can be harvested about it, not in the ranking.",
        },
        new()
        {
            Query = "view memory usage",
